...
export default function CheckoutPage() {
  ...
  const [addressData, setAddressData] = useState<AddressForm>({ email: "", firstName: "", lastName: "", phone: "", city: "", district: "", neighborhood: "", fullAddress: "", addressTitle: "" });
  ...
  useEffect(() => {
    const initCheckout = async () => {
      ...
      try {
        const settingsResponse = await fetch("/api/site-settings", { method: "GET", credentials: "include" });
      ...
      if (session) {
        ...
        const { data: addr, error: addrErr } = await supabase.from("addresses").select("*").eq("user_id", session.user.id).order("is_default", { ascending: false }).order("created_at", { ascending: false });
  ...
  const selectedAddress = useMemo(() => selectedAddressId ? savedAddresses.find((address) => address.id === selectedAddressId) || null : null, [savedAddresses, selectedAddressId]);
  const usageCountsByCouponId = useMemo(() => couponUsages.reduce((acc: Record<string, number>, usage) => { acc[usage.coupon_id] = (acc[usage.coupon_id] || 0) + 1; return acc; }, {}), [couponUsages]);

  const getCouponProblem = (coupon: CouponRow) => {
    if (!isMember) return "Kuponlar sadece üyeler için geçerlidir.";
    if (Number(cartTotal || 0) < Number(coupon.min_order_amount || 0)) return `Bu kupon için sepet tutarı en az ${formatMoney(coupon.min_order_amount)} ₺ olmalıdır.`;
    if (coupon.usage_limit_total !== null && coupon.usage_limit_total !== undefined && Number(coupon.used_count || 0) >= Number(coupon.usage_limit_total)) return "Bu kuponun toplam kullanım hakkı dolmuştur.";
    const userUsageCount = usageCountsByCouponId[coupon.id] || 0;
    if (userUsageCount >= Number(coupon.usage_limit_per_user || 1)) return "Bu kuponu daha önce kullandınız.";
    return null;
  };
  ...
  const remainingForFreeShipping = useMemo(() => calculateRemainingForFreeShipping(shippingSettings, subtotalAfterCoupon, shippingFee), [shippingSettings, subtotalAfterCoupon, shippingFee]);
  ...
  const filteredCities = useMemo(() => cities.filter((city: any) => city.name.toLocaleLowerCase("tr-TR").includes(citySearch.toLocaleLowerCase("tr-TR"))), [cities, citySearch]);
  const filteredDistricts = useMemo(() => districts.filter((district: any) => district.name.toLocaleLowerCase("tr-TR").includes(districtSearch.toLocaleLowerCase("tr-TR"))), [districts, districtSearch]);
  const filteredNeighborhoods = useMemo(() => neighborhoods.filter((neighborhood: any) => neighborhood.name.toLocaleLowerCase("tr-TR").includes(neighborhoodSearch.toLocaleLowerCase("tr-TR"))), [neighborhoods, neighborhoodSearch]);
  ...
  const handleCitySelect = (cityName: string) => {
    ...
    setDistricts(selectedCity ? selectedCity.districts.sort((a: any, b: any) => a.name.localeCompare(b.name, "tr")) : []);
    setAddressData((prev) => ({ ...prev, city: cityName, district: "", neighborhood: "" }));
    setNeighborhoods([]); setShowCitySelect(false); setCitySearch("");
  };
  ...
  const handleNeighborhoodSelect = (neighborhoodName: string) => {
    ...
    setShowNeighborhoodSelect(false); setNeighborhoodSearch("");
  };

  const applyCoupon = (coupon: CouponRow) => {
    const problem = getCouponProblem(coupon);
    if (problem) { showNotice(problem, "error"); return; }
    const discount = calculateCouponDiscount(coupon, Number(cartTotal || 0));
    if (discount <= 0) { showNotice("Bu kupon mevcut sepet için indirim oluşturmuyor.", "error"); return; }
    setSelectedCoupon(coupon); setCouponCode(coupon.code); showNotice(`${coupon.code} kuponu uygulandı.`, "success");
  };

  const handleApplyCouponCode = () => {
    const code = normalizeCouponCode(couponCode);
    if (!isMember) { showNotice("Kupon kullanmak için giriş yapmalısınız.", "error"); return; }
    if (!code) { showNotice("Lütfen kupon kodu girin.", "error"); return; }
    const foundCoupon = coupons.find((coupon) => coupon.code === code);
    if (!foundCoupon) { showNotice("Kupon bulunamadı veya aktif değil.", "error"); return; }
    applyCoupon(foundCoupon);
  };

  const removeCoupon = () => { setSelectedCoupon(null); setCouponCode(""); showNotice("Kupon kaldırıldı.", "info"); };
  ...
  const handleSaveAddressModal = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (validationError) { showNotice(validationError, "error"); return; }
    setIsSavingAddress(true);
    try {
      const cleanedAddress = { title: normalizeText(addressData.addressTitle), first_name: normalizeText(addressData.firstName), last_name: normalizeText(addressData.lastName), phone: normalizePhone(addressData.phone), city: addressData.city, district: addressData.district, neighborhood: addressData.neighborhood, full_address: normalizeText(addressData.fullAddress) };
      if (user) {
        const { data: inserted, error: insErr } = await supabase.from("addresses").insert([{ user_id: user.id, ...cleanedAddress, is_default: savedAddresses.length === 0 }]).select("*").single();
        if (insErr) throw insErr;
        if (inserted) { setSavedAddresses((prev) => [inserted as AddressRow, ...prev]); setSelectedAddressId(inserted.id); }
      } else {
  ...
  const validateBeforePay = () => {
    ...
    if (!selectedAddress.city || !selectedAddress.district || !selectedAddress.neighborhood || !selectedAddress.full_address) return "Seçili teslimat adresi eksik. Lütfen yeni adres ekleyin.";
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6 lg:gap-10">
        <div className="flex-1 space-y-5 md:space-y-6">
          <section className="bg-white p-5 md:p-6 rounded-3xl border border-gray-100 shadow-sm">
            <button type="button" onClick={() => setIsCartOpen(!isCartOpen)} className="w-full flex items-center justify-between cursor-pointer group text-left"><div className="flex items-center gap-3 md:gap-4"><span className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center font-black text-sm">1</span><h2 className="text-base md:text-lg font-black uppercase tracking-tighter text-black">Sepetimdeki Ürünler ({cartItems.length})</h2></div><div className="flex items-center gap-3"><span className="hidden sm:block text-[11px] font-black text-gray-400 uppercase tracking-widest">{formatMoney(cartTotal)} ₺</span><div className={`w-8 h-8 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-gray-100 transition-all duration-300 ${isCartOpen ? "rotate-180" : ""}`}>▼</div></div></button>
            {isCartOpen && <div className="mt-6 border-t border-gray-50 pt-6 space-y-4">{cartItems.map((item: any, index: number) => { const displayImage = item.images?.[0] || item.image || "/logo.jpeg"; const itemPrice = Number(item.discount_price) > 0 ? Number(item.discount_price) : Number(item.price || 0); return <div key={`${item.id}-${index}`} className="flex gap-4 items-center"><div className="w-14 h-14 bg-gray-50 rounded-xl border border-gray-100 overflow-hidden flex-shrink-0"><img src={displayImage} className="w-full h-full object-cover mix-blend-multiply" alt={item.name || "Ürün"} /></div><div className="flex-1 min-w-0"><p className="text-[10px] font-black uppercase leading-tight line-clamp-2">{item.name}</p><p className="text-[10px] font-bold text-gray-400 mt-1">{item.quantity || 1} Adet</p></div><p className="text-xs font-black">{formatMoney(itemPrice * (item.quantity || 1))} ₺</p></div>; })}</div>}
          </section>

          {!user && !checkoutMode && <section className="bg-white p-5 md:p-7 rounded-3xl border border-gray-100 shadow-sm"><div className="flex items-center gap-4 mb-5"><span className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center font-black text-sm">2</span><h2 className="text-base md:text-lg font-black uppercase tracking-tighter text-black">Nasıl Devam Etmek İstersiniz?</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><button type="button" onClick={() => router.push("/login?redirect=/checkout")} className="rounded-2xl bg-black text-white p-5 text-left active:scale-[0.98] transition-all"><p className="text-sm font-black uppercase tracking-widest">Giriş Yap / Kaydol</p><p className="text-xs font-medium text-white/70 mt-2">Kuponlar, kayıtlı adresler ve sipariş takibi için önerilir.</p></button><button type="button" onClick={() => setCheckoutMode("guest")} className="rounded-2xl bg-gray-50 border border-gray-200 text-black p-5 text-left active:scale-[0.98] transition-all"><p className="text-sm font-black uppercase tracking-widest">Üye Olmadan Devam Et</p><p className="text-xs font-medium text-gray-500 mt-2">E-posta, telefon ve teslimat adresiyle hızlıca ödeme yapın.</p></button></div></section>}
