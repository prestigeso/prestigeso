...
export const OrderConfirmation: React.FC<Readonly<OrderConfirmationProps>> = ({
  ...
  <div style={{ fontFamily: "sans-serif", color: "#333", maxWidth: "600px", margin: "0 auto", padding: "20px" }}>
    ...
    <h2 style={{ fontSize: "20px", fontWeight: "normal", marginBottom: "20px" }}>
      Merhaba {customerName},
    </h2>
    
    <p style={{ fontSize: "15px", lineHeight: "1.6" }}>
      Siparişiniz başarıyla alınmıştır. Bizi tercih ettiğiniz için teşekkür ederiz. 
      Siparişiniz hazırlandığında ve kargoya verildiğinde sizi tekrar bilgilendireceğiz.
    </p>

    <div style={{ margin: "30px 0", padding: "20px", backgroundColor: "#f9f9f9", borderRadius: "8px" }}>
      <h3 style={{ marginTop: 0, fontSize: "16px", textTransform: "uppercase" }}>Sipariş Özeti ({orderId})</h3>
      ...
      {items.map((item, index) => (
        <div key={index} style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
          ...
          <span style={{ fontSize: "14px", fontWeight: "bold" }}>{Number(item.price * item.quantity).toLocaleString("tr-TR")} ₺</span>
        </div>
      ))}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "10px" }}>
        ...
        <strong style={{ fontSize: "16px", color: "#e53e3e" }}>{Number(totalAmount).toLocaleString("tr-TR")} ₺</strong>
      </div>
    </div>
