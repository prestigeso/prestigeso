...
export function useMessagingActions({
  ...
  const handleSendMessageReply = async (messageId: number) => {
    ...
    const { error } = await adminDb({
      action: "update",
      table: "messages",
      data: { answer: replyText, answered_at: now },
      filters: [{ column: "id", op: "eq", value: messageId }],
    });
    if (error) {
  ...
  const handleSendQuestionReply = async (questionId: number) => {
    ...
    const { error } = await adminDb({
      action: "update",
      table: "questions",
      data: { answer: qReplyText, answered_at: now },
      filters: [{ column: "id", op: "eq", value: questionId }],
    });
    if (error) {
      showToast("Cevap gönderilemedi: " + error, "error");
      return;
  ...
  const handleToggleQuestionApproval = async (
    ...
    showToast(
      newStatus ? "Soru yayına alındı." : "Soru yayından kaldırıldı.",
      "success",
    );
  };
